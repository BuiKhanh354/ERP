...
(function initDeleteEmployee() {
  ...
  function handleDeleteEmployeeFromElement(element, event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    const employeeId = element.getAttribute('data-employee-id');
    const employeeName = element.getAttribute('data-employee-name') || 'nhân sự này';
    handleDeleteEmployee(employeeId, employeeName, event);
  }

  async function handleDeleteEmployee(employeeId, employeeName, event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    
    // Xử lý employeeName để đảm bảo là string
    const nameStr = String(employeeName || 'nhân sự này');
    
    const confirmed = await window.showConfirm(
      `Bạn có chắc chắn muốn xóa nhân sự <strong>${nameStr}</strong>?<br><small class="text-muted">Hành động này không thể hoàn tác. Tất cả dữ liệu liên quan đến nhân sự này sẽ bị xóa.</small>`,
    ...
    form.method = 'POST';
    form.action = `/resources/${employeeId}/delete/`;
  ...
  // gắn vào global để onclick gọi được
  window.handleDeleteEmployee = handleDeleteEmployee;

  document.addEventListener('DOMContentLoaded', function() {
    const urlParams = new URLSearchParams(window.location.search);
    const deletedEmployee = urlParams.get('deleted');
    if (deletedEmployee) {
      const decodedName = decodeURIComponent(deletedEmployee);
      setTimeout(function() {
        window.showNotification('success', `Đã xóa nhân sự "${decodedName}" thành công.`, 'Thành công');
      }, 300);
      const newUrl = window.location.pathname + (window.location.search.replace(/[?&]deleted=[^&]*/, '').replace(/^&/, '?') || '');
